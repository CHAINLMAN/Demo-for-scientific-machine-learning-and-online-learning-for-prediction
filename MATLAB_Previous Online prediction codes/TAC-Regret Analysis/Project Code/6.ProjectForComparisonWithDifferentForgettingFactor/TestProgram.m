...
p = ceil(beta * log(N/2));
gamma = 0.6;
gamma2 = [0.9,0.99,0.999,0.9999,1];
lgamma = 5;


PerformKF = zeros(N-Tini,times);
PerformReg = zeros(N-Tini,times);
PerformForgetBad = zeros(N-Tini,times*lgamma);
...
for i = 1:times
   ...
   for l = 1:lgamma
      ...
      [yFogB] = RegressionFilterWithForgetting(y1,beta,lambda,Tini,Epoh,gamma2(l));
   
      PerformForgetBad(1,l*times-times+i) = norm(y1(:,Tini+1)-yFogB(:,Tini+1))^2;

      for j = 2: N - Tini
        
          PerformForgetBad(j,l*times-times+i) = PerformForgetBad(j-1,l*times-times+i) + norm(y1(:,j+Tini)-yFogB(:,j+Tini))^2;
...
PerformanceFogB = zeros(N-Tini,lgamma);
for j = 1:lgamma
   
    PerformanceFogB(:,j) = 1/times*sum(PerformForgetBad(:,j*times-times+1:j*times),2);
...
for j = 1:lgamma
   ...
   semilogy(tag,PerformanceFogB(:,j)-PerformanceKF);
