...
% Acor = 0.1 * rand(3);
% A = kron(eye(2),Ablk)+kron([0,1;1,0],Acor);
% A = 1/(max(abs(eig(A)))+1e-5) * A;
% 
% C1 = [1,0,0];
% C = kron([1,0], C1);
% Ca = kron([0,1],C1);
...
for l = 1:H
   ...
   [y2] = observationProcess(xstate,Ca,R);
   ...
   [yRegDelay] = RegressionFilterWithDelay(y1,y2,beta,lambda,Tini,Epoh,1,l,1);
   ...
   PerformRegDelay{1,l}(i,:) = PTemp - PKF;
...
tag = 1:1:N-Tini;
plot(tag,PerformanceRegLoc)
plot(tag,PerformanceRegDelayLoc)
% figure(3)
% hold on
